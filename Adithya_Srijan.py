...
#check the store whihc have no demand 
...
for dc in Dc_num: 
    for store in Store_num:
        mydough[store,dc]= domPizza.addVar(obj = ((Cost[dc]*miles[store,dc])/9900.0), 
...
for store in Store_num:
    ...
    myConstrs[constrName] = domPizza.addConstr(quicksum(mydough[store,dc] for dc in Dc_num) 
                                           >= int(round(Demand_Store[store])), 
                                           name = constrName)
domPizza.update()
##%% create the supple constraint

for dc in Dc_num:
    constrName = '%s_limits' % dc
    myConstrs[constrName] = domPizza.addConstr(quicksum(mydough[store,dc] for store in Store_num) 
                                           <= ((4.0*supply_dc[dc])/7), 
                                           name = constrName)
domPizza.update()
...
if domPizza.Status == GRB.OPTIMAL:
    ...
    domSolution = []
    costSolution=[]
    for store,dc in mydough:
        if mydough[store,dc].x > 0:
            domSolution.append((store,dc, mydough[store,dc].x))
    for cost in obj_values:
            costSolution.append((obj_values[cost]))

sqlString = """
            CREATE TABLE IF NOT EXISTS tbldom
            ( Store_Number STRING, Distribution_Num STRING,
            Number_of_Piza  INTEGER);
            """
...
myData_test=[]
SQLString="""SELECT SUM(Avg_Demand) FROM tblAvgDemand ;"""
myCursor.execute(SQLString)
myData_test=myCursor.execute(SQLString).fetchall() # store the values of avg daemaand in the dataframe myData
myConn.commit()
